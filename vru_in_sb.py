...
def kocka():
	...
	for v in range(velikost):
		for s in range(velikost):
			p = Pika(razdalja*s+150,razdalja*v+10,True)
			vse_pike.append(p)
			all_sprites.add(p)

	for i in range(len(vse_pike)):
		if i >= velikost:
			l = Palcka(vse_pike[i],vse_pike[i-velikost])
			all_sprites.add(l)
			vse_palcke.append(l)

		if i% velikost != 0:
			l = Palcka(vse_pike[i],vse_pike[i-1])
			all_sprites.add(l)
			vse_palcke.append(l)

			if i >= velikost:
				l = Palcka(vse_pike[i],vse_pike[i-velikost-1])
				all_sprites.add(l)
				vse_palcke.append(l)
		
		if i>velikost and i%velikost != (velikost-2):
			l = Palcka(vse_pike[i],vse_pike[i+2-velikost])
			all_sprites.add(l)
			vse_palcke.append(l)

		l = Palcka(vse_pike[velikost-1],vse_pike[-velikost])
		all_sprites.add(l)
		vse_palcke.append(l)

		l = Palcka(vse_pike[0],vse_pike[-1])
		all_sprites.add(l)
		vse_palcke.append(l)
...
def kocka2():
	...
	l = Palcka(vse_pike[0],vse_pike[velikost-1])
	all_sprites.add(l)
	vse_palcke.append(l)
	l = Palcka(vse_pike[velikost-1],vse_pike[-1])
	all_sprites.add(l)
	vse_palcke.append(l)
	l = Palcka(vse_pike[0],vse_pike[-velikost])
	all_sprites.add(l)
	vse_palcke.append(l)
	l = Palcka(vse_pike[-velikost],vse_pike[-1])
	all_sprites.add(l)
	vse_palcke.append(l)
...
class Pika(pg.sprite.Sprite):
	def __init__(self,x,y,movable):
		...
		if self.movable:
			self.color = (0,50,150)#(50,200,50)#WHITE
			self.width = 12
		else:
			self.width = 16
		...
		if not self.movable or prikazuj_pike:
...
class Palcka(pg.sprite.Sprite):
	def __init__(self,pika1,pika2): 
		...
		self.color = (50,100,200)#(100,200,100)
...
kocka_gumb = tk.Button(canvas,text="kocka",command=kocka2,bg="white",bd =2,padx=4,pady=4,font=Gfont)
kocka_gumb.place(x=155, y=120)
...
while running:
	...
	barva_crte = (120,120,120)
	pg.draw.line(screen, barva_crte, (20,HEIGHT-50), (WIDTH-20,HEIGHT-50))
	pg.draw.line(screen, barva_crte, (20,20), (WIDTH-20,20))
	pg.draw.line(screen, barva_crte, (20,20), (20,HEIGHT-50))
	pg.draw.line(screen, barva_crte, (WIDTH-20,20), (WIDTH-20,HEIGHT-50))
